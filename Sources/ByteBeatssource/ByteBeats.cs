        ...
        if (waveOutOpen(out IntPtr hwo, 0xFFFFFFFF, ref fmt, IntPtr.Zero, IntPtr.Zero, 0) == 0)
        {
            ...
            while (true)
            {
                ...
                for (int i = 0; i < bufferSize; i++)
                {
                    ...
                    if (faseAudio == 1)
                        buffer[i] = (byte)((t * (t >> 8 | t >> 4) & 8 & t >> 5) ^ (t & t >> 2 | t >> 1)); // Bytebeat 1
                    else if (faseAudio == 2)
                        buffer[i] = (byte)(t * ((t >> 8 | t >> 8) & (5 & t >> 4))); // Bytebeat 2
                    else if (faseAudio == 3)
                        buffer[i] = (byte)(t * ((t >> 1 | t >> 6) & (4 & t >> 3))); // Bytebeat 3
                    else if (faseAudio == 4)
                        buffer[i] = (byte)(t * ((t >> 1 | t >> 5) & (1 & t >> 4))); // Bytebeat 4
                    else
                        buffer[i] = (byte)((t * 1 & t >> 2) | (t * 7 & t >> 3)); // Bytebeat 5
                    t++;
                }
...
public static class Program
{
    public static void Main(string[] args)
    {
        ...
        audio.Start();
        Console.WriteLine("The first ByteBeats credits for ArTicZera");
        Console.ReadLine();
    }
}